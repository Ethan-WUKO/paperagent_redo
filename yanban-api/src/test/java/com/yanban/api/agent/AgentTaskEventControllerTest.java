...
class AgentTaskEventControllerTest {
    ...
    void listEventsUsesCurrentUser() {
        ...
        when(service.listEvents(11L, "LITERATURE_SEARCH", 101L)).thenReturn(expected);

        List<AgentTaskEventResponse> actual =
                controller.listEvents(new JwtUser(11L, "alice"), "LITERATURE_SEARCH", 101L);

        assertThat(actual).isSameAs(expected);
        verify(service).listEvents(11L, "LITERATURE_SEARCH", 101L);
    }
}
