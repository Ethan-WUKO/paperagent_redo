...
class AgentStrategySelectorTest {
    ...
    @Test
    void naturalLanguagePlanWordsDoNotEnterPlanExecution() {
        AgentToolPolicyEngine.Decision toolPolicy = new AgentToolPolicyEngine.Decision(List.of("search_web"), 1, 1, "search");

        assertThat(selector.select("Please analyze this and propose a plan.", toolPolicy))
                .isEqualTo(AgentStrategy.SINGLE_STEP_REACT);
        assertThat(selector.isPlanReflectIntent("/plan reflection should not match"))
                .isFalse();
    }
}
