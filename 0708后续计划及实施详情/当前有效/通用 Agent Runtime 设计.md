...
> 文档状态：当前长期架构基线 1.0
> 初版时间：2026-07-10
> 最近复核：2026-07-15（总体方向保持有效；当前排期以《科研 Project Agent 第一版实施计划》为准）
...
策略选择器使用确定性特征和模型分类的组合，但最终由 Runtime 决策。

| 场景 | 推荐策略 |
|---|---|
| 常识解释、已有上下文总结 | DIRECT |
| 一次知识库搜索或单文件读取 | REACT |
| 目标不清，需要先探索 Project | REACT |
| 跨多个文件且有明确交付物 | PLAN_EXECUTE |
| 多个可并行且相互独立的子目标 | PLAN_EXECUTE，后续可拆 worker |
| 用户明确要求只分析、不修改 | READ_ONLY_REACT 或 READ_ONLY_PLAN |

不允许仅因为用户使用“计划”“分析”等词就机械进入 Plan 模式。策略选择需考虑工具数量、文件范围、写入风险、任务时长和成功条件。

执行策略与修改权限相互独立。例如 `PLAN_EXECUTE + ADVISE_ONLY` 可以完成复杂跨文件审查，但不能写文件；`REACT + PROPOSE_CHANGES` 可以生成单文件 patch，但不能直接应用。
...
1. 校验请求、权限和幂等键
2. 构建 ContextPackage
3. 选择 DIRECT / REACT / PLAN_EXECUTE
4. 创建预算和终止条件
5. 请求模型生成下一动作
6. 校验动作是否允许
...
长期目标是只读接入经过确认的长期记忆，不自动记住全部内容。最小 MVP 继续关闭长期记忆主链路注入，只保留现有设施和后续契约。
...
当前长期记忆服务虽然存在，但主 Agent 链路仍返回空上下文。恢复接入前必须先完成来源、权限、相关性和删除机制，不能简单打开全量注入。
...
5. Reflection 只在关键结果后按需使用，不能代替确定性验证。
6. MVP 默认串行，只承诺前端断开后继续的 L1 任务，不宣称服务重启恢复。
7. 安全写入是 MVP 后第一扩展，必须同时完成 ChangeSet、确认、hash 冲突检查、原子应用和应用后验证。
8. 通用异步托管、L2 恢复和多 Agent 在 MVP 后按独立门禁推进。
9. MVP 必须以真实 Project 科研任务和分场景硬性评测门槛验收。

当且仅当第 12 章的只读 MVP 闭环与第 11 章的发布门槛通过后，才进入安全写入扩展；安全写入单独验收后，才考虑自由多 Agent、完整沙箱和自动执行科研代码。
