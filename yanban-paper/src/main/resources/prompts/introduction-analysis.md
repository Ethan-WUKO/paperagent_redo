...
- Each citation slot must copy `sourceAnchor` as one exact, verbatim sentence from the supplied Introduction, including any LaTeX citation commands. Use an empty string only when no single source sentence corresponds to the claim.
  ...
  "citationSlots": [
    {
      ...
      "sourceAnchor": "the exact verbatim Introduction sentence containing that claim",
